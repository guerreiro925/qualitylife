...
<head>
<title><?php $id = $_GET['id']; $query  = mysql_query("SELECT * FROM categoria WHERE idCategoria ='$id' AND deletar != 1");
while($resultado = mysql_fetch_array($query)){
echo $resultado['nome'];
} 
?> | Qualidade de Vida</title>    
...
<body>
	...
	<div class="clearfix"></div>
			  ...
			  <div class="container-fluid">
				...
				<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
				  <ul class="nav navbar-nav">
					...
					<li class="dropdown">
					  <a class="active" href="#" class="dropdown-toggle" style="font-family: 'Raleway', sans-serif;" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Categoria <span class="caret"></span></a>
		...
		<div class="agileits Blog">
			<div class="container">
				<h2 class="agile-inner-title">NOTÍCIAS - <?php 
				$id = $_GET['id'];
				$query  = mysql_query("SELECT * FROM categoria WHERE idCategoria ='$id' AND deletar != 1");
							while($resultado = mysql_fetch_array($query)){
							echo $resultado['nome'];
							} 
							?></h2>
	...
	<div class="w3layouts-news">
			<div class="agile-its-news-grid">
				...
				<?php
				$id = $_GET['id'];
				$consulta= mysql_query("SELECT * FROM noticias WHERE idCategoria = '$id' AND deletar != 1 " );
					while($dados= mysql_fetch_array($consulta)){
...
$date = $dados['data'];
 ...
 <span><?php echo strftime("%d", strtotime($date));?></span></h4>
							<a href="noticia.php?id=<?php echo $dados['idNoticia'];?>"><i class="glyphicon glyphicon-tags" aria-hidden="true"></i>10</a>
						</div>
						<div class="w3ls-blog-leftr " style="float: right;">
							<a href="continuar.php?id=<?php echo $dados['idNoticia'];?>"><img src="<?php echo $dados['imagem'];?>" alt=" " class="img-responsive" /></a>
							<p><?php echo $dados['resumo'];?></p>
							<ul>
								<li><a href="continuar.php?id=<?php echo $dados['idNoticia'];?>"><i class="glyphicon glyphicon-tags" aria-hidden="true"></i><?php $query  = mysql_query("SELECT * FROM categoria WHERE idCategoria ='$idCategoria'");
							while($resultado = mysql_fetch_array($query)){
							echo $resultado['nome'];
							} 
							?></a></li>
